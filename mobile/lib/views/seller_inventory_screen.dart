...
class SellerInventoryScreen extends StatefulWidget {
  ...
  const SellerInventoryScreen({super.key, this.showLowStockOnly = false});
...
class _SellerInventoryScreenState extends State<SellerInventoryScreen> {
  ...
  void _changeStock(dynamic item, int delta) {
    setState(() {
      final realIndex = _inventory.indexOf(item);
      if (realIndex == -1) return;
      
      final currentStock = int.tryParse(_inventory[realIndex]['stock_quantity']?.toString() ?? '0') ?? 0;
      final newStock = currentStock + delta;
      if (newStock >= 0) {
        _inventory[realIndex]['stock_quantity'] = newStock.toString();
      }
    });
  ...
  Future<void> _deleteItem(dynamic item) async {
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('${item['medicine']?['name'] ?? 'Bu ürün'} stok listesinden tamamen kaldırılacak. Emin misiniz?', style: GoogleFonts.inter()),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text('İptal', style: GoogleFonts.inter(color: Colors.grey))),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: Text('Sil', style: GoogleFonts.inter(color: AppTheme.outOfStock, fontWeight: FontWeight.w700))),
        ],
      ),
    );
    ...
    try {
      final token = await AuthService.getToken();
      if (token != null) {
        await ApiService.deleteInventoryItem(token, item['id']);
        if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Hata: $errorMsg'), backgroundColor: AppTheme.outOfStock),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: GestureDetector(
        ...
        floatingActionButton: widget.showLowStockOnly ? null : FloatingActionButton(
          onPressed: () async {
  ...
  Widget _buildContent() {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                widget.showLowStockOnly ? 'Stoğu Azalan Ürün Yok' : 'Envanter Boş',
    ...
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (context, index) => Padding(
          padding: const EdgeInsets.only(bottom: 12),
          child: _buildInventoryCard(_inventory[index]),
        ),
                            ...
                            child: Center(
                              child: TextField(
                                ...
                                onChanged: (value) {
                                  final realIndex = _inventory.indexOf(item);
                                  if (realIndex != -1) {
                                    _inventory[realIndex]['stock_quantity'] = value;
                                  }
                                },
                                style: GoogleFonts.inter(
                                  fontWeight: FontWeight.w800,
            ...
            child: ElevatedButton(
              onPressed: () async {
                ...
                try {
                  ...
                  final errorMsg = e.toString().replaceAll('Exception: ', '');
                  if (errorMsg.contains('401') && context.mounted) {
                    Provider.of<AuthProvider>(context, listen: false).logout();
                    Navigator.of(context).popUntil((route) => route.isFirst);
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: const Text('Oturumunuz sonlandırıldı. Lütfen tekrar giriş yapın.'),
